...
def sync_all_data(watchlist):
    ...
    for _, row in watchlist.iterrows():
        ...
        try:
            ...
            if hist.empty:
                report["err_y"] = "Yahoo 回傳空數據 (Rate Limited)"
            else:
                # 使用 info.get 確保相容性，不使用已失效的 FastInfo 屬性
            ...
            report["err_y"] = f"Yahoo 錯誤: {str(ey)}"
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                report["err_f"] = "FinMind 無查得數據"
        except Exception as ef:
            report["err_f"] = f"FinMind 錯誤: {str(ef)}"
