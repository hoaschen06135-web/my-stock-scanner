...
new_sid = st.sidebar.text_input("輸入股票代號", placeholder="例如: 2330")
            ...
            with st.spinner("正在同步至 Google Sheets..."):
                ...
                if not match.empty:
                    ...
                    if new_sid in existing_data.values:
                        ...
                        st.success(f"已成功新增: {new_sname}")
                        st.rerun()
                else:
                    st.error("代號有誤，找不到該股票")
    ...
    with st.container(border=True):
        ...
        with col_main:
            ...
            if not df_daily.empty:
                # 修復 image_24732a.png 的 KeyError 問題
                target_info = stock_info[stock_info['stock_id'] == sid]
                # 檢查多個可能的股數欄位名稱
                total_shares = 0
                for col in ['public_shares', 'shares', 'issued_shares']:
                    if col in target_info.columns:
                        total_shares = target_info[col].values[0]
                        break
